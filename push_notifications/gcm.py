...
def _fcm_send(data, content_type, application_id):
	...
	if SETTINGS.get("GCM_CUSTOM_ACTIVE"):
		...
		# decoding the encoded data and grabbing the app_type.
		# ast package is use to convert string provided bt decode to dictionary.
		else:
			app_type = ast.literal_eval(data.decode('utf-8'))
			app_type = app_type["data"]["app_type"]
		if not app_type:
			raise ImproperlyConfigured(
				'Please provide app_type')
		# add more app if required in if-else ladder
		elif app_type == 'app1':
			key = SETTINGS.get("GCM_APP1_KEY")
		elif app_type == 'app2':
			key = SETTINGS.get("GCM_APP2_KEY")
	# if custom gcm mode is not active then send the default app api key.
	else:
		key = SETTINGS.get("GCM_APP2_KEY")
	if not key:
		raise ImproperlyConfigured(
